...
const TABS = {
  // The five columns you actually type — name, email, job_role, category,
  // notes — come first so the sheet is usable without scrolling sideways.
  // `notes` is free text handed to the model as extra context ("met at the
  // PDEU fair", "strong on React, weak on backend"), which is the cheapest way
  // to make a generated email specific. Everything after `stage` is written by
  // the app; leave those cells empty when you add a candidate by hand.
  Applicants: [
